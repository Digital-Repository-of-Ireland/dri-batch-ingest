...
class DriBatchIngest::ProcessBatch
  ...
  def self.perform(batch_id, media_object_ids = nil)
    ...
    media_objects.each do |mo|
      ...
      ingest_message = process_media_object(media_object, batch.collection_id)
      
      Resque.enqueue(::ProcessBatchIngest, current_user.id, collection_id, ingest_message.to_json)
    end
  end
