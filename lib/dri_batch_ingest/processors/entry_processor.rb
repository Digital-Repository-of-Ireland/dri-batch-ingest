  ...
  module BatchIngest
    module Processors
      class EntryProcessor < Avalon::Batch::Entry
        ...
        def media_object
          @media_object ||= DRI::BatchIngest::MediaObject.new(collection: @manifest.package.collection)
        end
              
        def process!(opts={})
          batch = DRI::BatchIngest::IngestBatch.find(opts['batch'])
          ...
          @files.each do |file_spec|
            master_file = DRI::BatchIngest::MasterFile.new
